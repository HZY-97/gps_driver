...
int main()
{
    int clientSocket = socket(AF_INET, SOCK_STREAM, 0); // 创建套接字，返回套接字描述符
    if (clientSocket == -1)
    {
    ...
    serverAddr.sin_port = htons(SERVER_PORT);              // 设置端口号，使用网络字节序（大端序）
    inet_pton(AF_INET, SERVER_IP, &(serverAddr.sin_addr)); // 将IP地址字符串转换为二进制格式
