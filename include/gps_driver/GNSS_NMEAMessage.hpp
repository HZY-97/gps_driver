 ...
 public:
  ...
  Comnav_Decode_Error _Decode(string& Message) override {
    ...
    while (pos1 <= (int)Body.length()) {
      ...
      component.push_back(string(Body.substr(pos1, pos2 - pos1)));
    ...
    if (checksum(Message.substr(0, starpos)) !=
        stoi(Message.substr(starpos + 1, 2), 0, 16)) {
      return Comnav_Decode_DecodeError;
    }
...
class GNSS_GPGGAMessage : public GNSS_NMEAMessage {
 public:
  ...
  GNSS_GPGGAMessage(const GNSS_NMEAMessage& p) : GNSS_NMEAMessage(p.Message) {
    Decode();
  }
...
class GNSS_GPYBMMessage : public GNSS_NMEAMessage {
 public:
  ...
  GNSS_GPYBMMessage(const GNSS_NMEAMessage& p) : GNSS_NMEAMessage(p.Message) {
    Decode();
  }
...
class GNSS_GPCHCMessage : public GNSS_NMEAMessage {
 public:
  ...
  GNSS_GPCHCMessage(const GNSS_NMEAMessage& p) : GNSS_NMEAMessage(p.Message) {
    Decode();
  }
