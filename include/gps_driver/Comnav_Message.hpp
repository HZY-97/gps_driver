...
class Comnav_Message {
 ...
 private:
  Comnav_Message(string Message) : Message(Message){};
...
class Comnav_Send_Protocol_Message : virtual public Comnav_Send_Message {
 ...
 public:
  ...
  virtual string EncodeHead() {
    return "";
  };
  virtual string EncodeTail() {
    return "";
  };
};
...
class Comnav_Recv_Message : virtual public Comnav_Message {
 public:
  Comnav_Recv_Message(string Message) {
    this->Message = Message;
  };

 public:
  Comnav_Decode_Error DecodeResult = Comnav_Decode_NotStart;

 public:
  bool isSuccess() {
    return (DecodeResult == Comnav_Decode_Success);
  };

 public:
  string Message_Remained;

 public:
  int SyncHeadLength = 1;

 private:
  virtual int FindSyncHead() {
    return 0;
  };
 ...
 private:
  virtual Comnav_Decode_Error _Decode(string& Message) = 0;
 ...
 private:
  Comnav_Decode_Error DecodeGetResult() {
    try {
    start:
      if (Message.length() == 0) {
      ...
      if ((int)Message.length() > MaxLength + 16 * 1024) {
        Message = Message.substr(MaxLength + 16 * 1024,
                                 Message.length() - (MaxLength + 16 * 1024));
      }
      ...
      if (FindSyncHead() == -1) {
        Message_Remained =
            Message.substr(Message.length() - SyncHeadLength, SyncHeadLength);
      ...
      if (ret == Comnav_Decode_Success) {
        ...
        // Message_Remained = Message.substr(Total_Length,Message.length() -
        // Total_Length);
        ...
        // Message = Message_Remained;
        // DEBUG_ERR("Message.length() %d > %d" ,(int)Message.length(),
        // MaxLength);
        Message.erase(0, 1);
        goto start;
      } else if (ret == Comnav_Decode_MessageShort) {
        Message_Remained = Message;
        // DEBUG_INFO("Message_Remained.length() ==
        // %d",Message_Remained.length());
...
class GNSS_Message2 : public Comnav_Recv_Message {
 public:
  GNSS_Message2(string Message) : Comnav_Recv_Message(Message){};
