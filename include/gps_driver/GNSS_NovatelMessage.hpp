...
{public:
    ...
    int FindSyncHead() override
    {
        ...
        //DEBUG_INFO("GNSS_NovatelMessage::FindSyncHead() == %d", Message.find(string().append((char*)synchead,3),0));
        return Message.find(string().append((char*)synchead,3),0);
    }

    Comnav_Decode_Error _Decode(string& Message) override
    {
        if(Message.length() < 28) return Comnav_Decode_MessageShort;
        Comnav_BinaryDecoder decoder(Message);
        MessageID = decoder.Decode_Word(4);
        if((int)MessageID > 9999) return Comnav_Decode_DecodeError;
        int Length = decoder.Decode_Word(8);
        if(Length > 19999) return Comnav_Decode_DecodeError;
        if((int)Message.length() < Length + 32) return Comnav_Decode_MessageShort;
...
{public:
    ...
    GNSS_BESTPOSB(const GNSS_NovatelMessage& p) : GNSS_NovatelMessage(p.Message) { Decode();  }
