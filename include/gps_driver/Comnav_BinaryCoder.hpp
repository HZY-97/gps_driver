 ...
 *        使用 Decode_xxx(value) 将字节按照指定类型取出，xxx代表数据类型，默认小端，大端使用带Encode_xxx_Reverse(value)的方法
 * @details 支持解码int char string word long float double等，或使用Decode()打包任意类型数据
