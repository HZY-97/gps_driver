...
class Comnav_Uart {
 public:
  ...
  Comnav_Uart(string Port) : Port(Port), Baudrate(115200), opened(false) {
    open();
  };
  Comnav_Uart(string Port, int Baudrate)
      : Port(Port), Baudrate(Baudrate), opened(false) {
    open();
  };
  ...
  int write(string buff) {
    return ::write(fd, (unsigned char *)buff.c_str(), buff.length());
  };
